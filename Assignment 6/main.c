...
/* z value for ray */
enum { z = 1 };
//moving some variables outside of main so both main and parallel function can use them.
int nthreads;
scene_t scene;
float scaled_color[width][height][3];
void* parallelFunc(void* numThreads);
...
main( int argc, char **argv )
{
    ...
    /* multithreading is now supported :smile:
    if( nthreads != 1 )
    {
...
pthread_t threads[nthreads];
 ...
 int id[nthreads];//fixes a segmentation fault error

    int ret;
   
    for (int i = 0; i < nthreads; i++){
      id[i]=i;
      ret = pthread_create(&threads[i], NULL, parallelFunc, &id[i]);
      if (ret!=0){
...
Vec3 camera_pos;
    ...
    const double subsample_dy
        = halfSamples ? pixel_dy / ((double)halfSamples*2.0)
                      : pixel_dy;
    int start = (width/nthreads) * (*(int*)numThreads);
    int end;
      if((*(int*)numThreads) == (nthreads-1)){
	end = width;
      }
      else{
	end = (width/nthreads) * (*(int*)numThreads + 1);
      }
